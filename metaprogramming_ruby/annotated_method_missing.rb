  ...
  class Base
    ...
    class << self # Class methods
      ...
      def method_missing(method_id, *arguments, &block)
        if match = DynamicFinderMatch.match(method_id) # 3 different types of dynamic methods that execute strings of code
          # in the context of a class eval, defining class methods
          attribute_names = match.attribute_names
          super unless all_attributes_exists?(attribute_names)
          if match.finder? #Deals with simple dynamic finders
            finder = match.finder
            bang = match.bang? # Determines if the method ends with a bang
            self.class_eval %{ 
          ...
          elsif match.instantiator? # Deals with more complex variations such as find_or_create_by_...
            instantiator = match.instantiator # Determines if the method is a create or a find
            self.class_eval %{
              def self.#{method_id}(*args)
          ...
          attribute_names = match.attribute_names
          super unless all_attributes_exists?(attribute_names) # Likely raising a NoMethodError
          if match.scope?
            self.class_eval %{
              def self.#{method_id}(*args) 
                options = args.extract_options!
            ...
            }, __FILE__, __LINE__ # Current file/line number, for debugging/logging
            send(method_id, *arguments)
          end
        else
          super
      ...
      # Example of code generated by the method_missing method for find_all_by_description_and_completed

      # def self.find_all_by_description_and_completed(*args) 
      #   options = args.extract_options!
      #   attributes = construct_attributes_from_arguments([:description,:completed],args )
      #   finder_options = { :conditions => attributes }
      #   validate_find_options(options)
      #   set_readonly_option!(options)
